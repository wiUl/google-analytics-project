...
request = RunReportRequest(
    ...
    dimensions=[#dimensão do dado, por data, e página
        Dimension(name="date"),
...
for row in response.rows:
    dados.append({
        ...
        'endereco': row.dimension_values[1].value,
        'nome_pagina': row.dimension_values[2].value,
        'usuarios_ativos': row.metric_values[0].value,
        'sessoes': row.metric_values[1].value,
        'visualizacoes': row.metric_values[2].value,
        'taxa_engajamento': row.metric_values[3].value,
        'tempo_medio_sessao': row.metric_values[4].value,
    })
...
with pd.ExcelWriter(
    ...
    df.to_excel(writer, sheet_name="Paginas", index=False)
